...
Gitea is a lightweight, self-hosted Git service with pull requests, CI/CD via Gitea Actions, and a clean GitHub-like UI.
...
services:
  ...
  gitea-db:
    ...
    volumes:
      - /data/appdata/gitea-db:/var/lib/postgresql/data
    restart: unless-stopped
```

## Setup

1. Start the stack and open Gitea at `http://IBM:3000` to complete the installation wizard.
...
5. Push your homelab repo: `git remote add gitea ssh://git@git.srng.no:222/you/homelab.git && git push gitea main`.
